...
# API endpoint
api_url = "https://localhost:44346/api/TeamRecSplits"
...
def post_data_to_api(data, api_url):
    ...
    for team_data in data:
        print(f"Posting data for team: {team_data}")
        response = requests.post(api_url, json=team_data, headers=headers, verify=False)
        if response.status_code == 201:  # Assuming 201 is the success status code
            print(f"Successfully posted data for {team_data['Team']}")
        else:
            print(f"Failed to post data for {team_data['Team']} - Status code: {response.status_code}")
...
post_data_to_api(merged_data, api_url)
