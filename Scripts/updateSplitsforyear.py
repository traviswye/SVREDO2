...
# Function to read bbrefIDs from a text file
def read_bbrefIDs_from_file(file_path):
    with open(file_path, 'r') as file:
        bbrefIDs = [line.strip() for line in file.readlines()]
    return bbrefIDs
...
def post_to_api(session, endpoint, data, is_update=False):
    ...
    if is_update and 'Year' in data and 'Split' in data:
        ...
        url += f"/{data['bbrefID']}"  # General case for 1st inning or other endpoints
...
date = "24-09-09"  # Example date string
year = 2024

# Main script execution
#file_path = "everySPthisyear.txt"  # Update this with the path to your file
file_path = "samplePitcherlist.txt"  # Update this with the path to your file

# Read the bbrefIDs from the text file
bbrefIDs = read_bbrefIDs_from_file(file_path)


# Loop through each bbrefID and scrape & post data for each pitcher
for pitcher_id in bbrefIDs:
...
based on your comprehension of the data above... 
