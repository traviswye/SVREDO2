...
# Function to correctly extract the full team name and handle playoff character suffixes
def extract_full_team_name(team_name):
    # Remove trailing playoff indicators (z, w, x, y)
    if team_name[-1] in ['z', 'w', 'x', 'y']:
        team_name = team_name[:-1]
    
    # Handle special cases for teams with two words in their name
    if "Sox" in team_name:
        return " ".join(team_name.split()[-2:])
    if "Jays" in team_name:
        return " ".join(team_name.split()[-2:])
    
    # For other teams, return the full last word
...
def scrape_standard_standings(url):
    ...
    for division in all_divisions:
        ...
        for row in rows:
            ...
            try:
                team_name = extract_full_team_name(row.find('th').text.strip())  # Team name is inside <th>
                team_data = {
                    'Team': team_name,
                    ...
                    'Diff': int(cols[7].text.strip()) - int(cols[8].text.strip()),  # Correctly calculate RS - RA
                    'ExpectedRecord': cols[10].text.strip(),
                    'HomeRec': cols[11].text.strip(),
                    'AwayRec': cols[12].text.strip(),
                    'vs500Plus': cols[13].text.strip(),
                    'NextGame': cols[14].text.strip() if len(cols) > 14 else None,
