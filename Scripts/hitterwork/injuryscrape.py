...
def post_injury_data(bbrefId, injury_description, current_team):
    ...
    if response.status_code == 201:
        print(f"Successfully posted injury for bbrefId: {bbrefId}")
    else:
...
for team_section in team_sections:
    ...
    team_name = team_section.find('img')['alt']
    ...
    for row in injury_rows:
        ...
        status_cell = row.find_all('td')[2] if len(row.find_all('td')) > 2 else None
...
print(f"Total number of injured players: {total_players}")
