                        ...
                        for row in rows:
                            ...
                            if first_column and first_column.text.strip() == '1st inning':
                                ...
                                try:
                                    ...
                                    # Print the extracted doubles and triples for verification
                                    print(f"Doubles (2B) for pitcher {pitcher_id}: {doubles}")
                                    print(f"Triples (3B) for pitcher {pitcher_id}: {triples}")
                                    
                                    pitcher_1st_inning_data = {
                                        "bbrefID": pitcher_id,
                                        "Year": year,  # Add year to the payload
                                        ...
                                        "TwoB": doubles,  # Use the extracted doubles value
                                        "ThreeB": triples,  # Use the extracted triples value
...
def pitcher_1st_inning_exists(session, bbrefID, year):
    ...
    if response.status_code == 200:
        print(f"Pitcher {bbrefID} exists in the database for year {year}.")
    elif response.status_code == 404:
        print(f"Pitcher {bbrefID} does not exist in the database for year {year}.")
    else:
...
date = "24-08-22"  # Example date string
year = 2024
...
for pitcher_id in pitchers:
    ...
    try:
        ...
        if pitcher_1st_inning_data:
            if pitcher_1st_inning_exists(session, pitcher_id, year):
            ...
            print(f"No data found for pitcher {pitcher_id}. Skipping.")
