...
def download_html(url, path):
    ...
    try:
        response = scraper.get(url, headers=headers, verify=False)  # Disable SSL verification
        response.raise_for_status()
        with open(path, 'w', encoding='utf-8') as file:
            file.write(response.text)
        logging.info(f"Content downloaded from {url} to {path}")
    except cloudscraper.exceptions.CloudflareChallengeError as e:
        logging.error(f"Cloudflare challenge error for {url}: {e}")
    except cloudscraper.exceptions.CloudflareCode1020 as e:
        logging.error(f"Cloudflare blocked access to {url}: {e}")
...
def send_post_request(game_info, date):
    ...
    try:
        response = scraper.post(url, json=payload, verify=False)  # Disable SSL verification for localhost
...
def scrape_parent_url(parent_url):
    try:
        response = scraper.get(parent_url, verify=False)  # Disable SSL verification
        response.raise_for_status()
        ...
        logging.error(f"Cloudflare challenge error for {parent_url}: {e}")
    except cloudscraper.exceptions.CloudflareCode1020 as e:
        logging.error(f"Cloudflare blocked access to {parent_url}: {e}")
    except requests.HTTPError as e:
