...
# Set the current year globally for use in all API requests
CURRENT_YEAR = 2025
...
if __name__ == "__main__":
    try:
        print(f"Starting trailing game log splits processing for year {CURRENT_YEAR}")
        ...
        for bbrefid in bbrefids:
            print(f"Processing {bbrefid} for year {CURRENT_YEAR}...")
            process_and_post_trailing_gamelogs(bbrefid, CURRENT_YEAR)
            time.sleep(2.5)  # Pause for 2.5 seconds between calls

        print(f"Processing complete for {len(bbrefids)} players for year {CURRENT_YEAR}.")
