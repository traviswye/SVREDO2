...
def extract_game_info(game_summary, index=0, game_date=None):
...
def send_post_request(game_info, date_str):
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info(f"Successfully posted: {game_info['home_team']} vs {game_info['away_team']}")
            return True
        else:
            logger.error(f"Failed to post game preview: {response.status_code} - {response.text[:100]}")
            return False
    except requests.RequestException as e:
        logger.error(f"Request exception occurred: {e}")
        return False

def post_game_info(game_info, date_str, index):
    """Process and post a single game - for use with ThreadPoolExecutor"""
    logger.info(f"Processing game {index+1}: {game_info['away_team']} @ {game_info['home_team']}")
    result = send_post_request(game_info, date_str)
    return result
...
def scrape_previews_with_selenium(chromedriver_path):
    ...
    try:
        ...
        # Check if we're on the Cloudflare page, wait longer if needed
        wait_time = 0
        max_wait = 30  # Maximum time to wait for Cloudflare to resolve
        
        while "Just a moment" in driver.title and wait_time < max_wait:
            logger.info("Still on Cloudflare challenge page. Waiting longer...")
            time.sleep(5)
            wait_time += 5
        
        if "Just a moment" in driver.title:
            logger.error("Failed to bypass Cloudflare challenge after waiting")
            return
        ...
        # Extract all game information at once
        game_info_list = []
        for i, game_summary in enumerate(game_summaries):
            game_info = extract_game_info(game_summary, i+1, date_obj)
            if game_info:
                game_info_list.append((game_info, i))
        
        logger.info(f"Extracted information for {len(game_info_list)} games")
        
        # Use ThreadPoolExecutor to post games in parallel
        successful_posts = 0
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(post_game_info, game_info, date_str, i) 
                for game_info, i in game_info_list
            ]
        ...
        logger.info(f"Successfully posted {successful_posts} out of {len(game_info_list)} game previews")
